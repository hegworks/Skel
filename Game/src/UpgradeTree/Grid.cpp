...
Grid::Grid()
{
	...
	m_gridScaleSurface = new ScaleSurface(m_totalLength.x, m_totalLength.y, false, m_screen->GetSize());
	m_gridScaleSurface->GetSurface()->Clear(0xff00ff00);
	...
	for(int row = 0; row < m_count.y; ++row)
	{
		for(int col = 0; col < m_count.x; ++col)
		{
			...
			m_nodeSurface->CopyTo(pos.x, pos.y, *m_gridScaleSurface->GetSurface());
...
void Grid::Draw()
{
	...
	const int2 gridDrawPosOnScreen = m_gridScaleSurface->GetDrawPosOnScreen();
	m_gridScaleSurface->GetSurface()->CopyToScaledFast(gridDrawPosOnScreen.x, gridDrawPosOnScreen.y, *m_screen, m_gridScaleSurface->GetScale());

	const float2 gridCenterOnScreen = m_gridScaleSurface->GetCenterOnScreen();
	...
	const int2 gridTopLeftOnScreen = ToInt2(m_gridScaleSurface->GetSurfaceToScreen(float2(0, 0)));
	...
	const int2 gridBottomRightOnScreen = ToInt2(
		m_gridScaleSurface->GetSurfaceToScreen(ToFloat2(m_gridScaleSurface->GetSize())));
...
void Grid::Update(float deltaTime)
{
	...
	if(input.IsKeyDown(GLFW_KEY_D)) m_gridScaleSurface->m_origin.x -= deltaTime * 2000.0f;
	if(input.IsKeyDown(GLFW_KEY_A)) m_gridScaleSurface->m_origin.x += deltaTime * 2000.0f;
	if(input.IsKeyDown(GLFW_KEY_W)) m_gridScaleSurface->m_origin.y += deltaTime * 2000.0f;
	if(input.IsKeyDown(GLFW_KEY_S)) m_gridScaleSurface->m_origin.y -= deltaTime * 2000.0f;
	...
	m_gridScaleSurface->ZoomAtScreenCoord(mousePosOnScreen, zoomFactor, 0.05f, 1.8f);
}
