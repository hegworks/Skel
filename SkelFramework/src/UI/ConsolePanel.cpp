...
skel::ConsolePanel::ConsolePanel()
{
	m_callbackSink = std::make_shared<spdlog::sinks::callback_sink_mt>(
		[this](const spdlog::details::log_msg& msg)
		{
			...
			m_formatter.format(msg, buf);

			ImVec4 color{1, 1, 1, 1};
			switch(msg.level)
			{
			case spdlog::level::trace: color = ImVec4(.6f, .6f, .6f, 1);
				break;
			case spdlog::level::debug: color = ImVec4(.7f, .7f, .7f, 1);
				break;
			case spdlog::level::info: color = ImVec4(.2f, 1.f, .2f, 1);
				break;
			case spdlog::level::warn: color = ImVec4(1.f, .8f, .2f, 1);
				break;
			case spdlog::level::err: color = ImVec4(1.f, .2f, .2f, 1);
				break;
			case spdlog::level::critical: color = ImVec4(1.f, 0.f, 0.f, 1);
				break;
			default: break;
			}
	...
	// m_callbackSink->set_formatter(std::make_unique<spdlog::pattern_formatter>("%v%$"));
	...
	RegisterCommand("clear", [this](auto&&)
	                {
		                m_Buffer.clear();
		                return true;
	                },
...
void ConsolePanel::ToggleDetailedFormat()
{
	m_isDetailedFormat = !m_isDetailedFormat;
	if(m_isDetailedFormat)
	{
		m_formatter.set_pattern("%^[%T] [%l] %n: %v%$");
	}
	else
	{
		m_formatter.set_pattern("%v%$");
	}
}
